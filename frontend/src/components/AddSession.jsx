...
const AddSession = () => {
  ...
  const getEmploye = async () => {
    const users = await axios.get(
      "http://localhost:8080/api/employe/allemployes"
    );
  ...
  const getFormation = async () => {
    const formations = await axios.get(
      "http://localhost:8080/api/formation/allformation"
    );
  ...
  const getOrganisme = async () => {
    const organismes = await axios.get(
      "http://localhost:8080/api/organisme/allorganisme"
    );
  ...
  const handleSubmit = async (e) => {
    ...
    const session = await axios.post(
      `http://localhost:8080/api/session/createsession`,
      formData
    );
